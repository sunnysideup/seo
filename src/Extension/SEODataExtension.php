...
class SEODataExtension extends DataExtension
{
    ...
    private static $add_self_canonical = true;
    ...
    public function updateCMSFields(FieldList $fields)
    {
        ...
        if (!$fields->fieldByName('Root.SEO.SEOFields_Container')) {
    ...
    public function MetaTagCollection($raw = false)
    {
        ...
        if (empty($canonical) && self::config()->get('add_self_canonical') && method_exists($record, 'AbsoluteLink')) {
    ...
    private function getStructuredDataProperties()
    {
        ...
        return ($type) ? $type->getProperties() : null;
    }
    ...
    private function processSchemaFields(BaseType $schema, $data)
    {
        foreach ($data as $property => $value) {
            ...
            } elseif (is_array($value)) { // this is a reference type
                if (empty($value['@type'])) {
                    throw new \Exception(
